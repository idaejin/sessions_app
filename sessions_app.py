...
if uploaded_files:
    ...
    if all_data:
        ...
        with tabs[2]:
            ...
            st.dataframe(filtro[['PROFESOR', 'TIPO_P.', 'PROGRAMA', 'SESSIONS']])
            st.markdown(f"🎯 **Total sesiones filtradas:** {filtro['SESSIONS'].sum()}")
            st.download_button("📥 Descargar resultados combinados", filtro.to_csv(index=False), file_name="filtro_combinado.csv")
